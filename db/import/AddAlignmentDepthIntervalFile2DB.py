...
Examples:
    %s --db_user yh --schema public
        -i folderReduce/63_alignments_CAE5_depth_GADAOut_minSegLength1000.tsv.gz
        --methodShortName PopSabaeusCoverageOn3488
        --alignmentIDList 6115-6177 --chromosome CAE5 --format tsv
...
class AddAlignmentDepthIntervalFile2DB(ParentClass):
    ...
    option_default_dict.update({
        ('chromosome', 0, ): ['', '', 1, 
            'which chromosome is the data from', ],\
        ('alignmentIDList', 0, ): ['', '', 1, 
            'coma/dash-separated list of alignment IDs, '
            'used to verify/create AlignmentDepthIntervalMethod', ],\
        ('methodShortName', 1, ):[None, 's', 1, 
            'column short_name of AlignmentDepthIntervalMethod table,'
            'will be created if not present in db.'],\
        ('format', 1, ):['tsv', '', 1, 
            'format for AlignmentDepthIntervalFile entry'],\
        })
    ...
    def run(self):
        ...
        alignmentList = self.db_main.getAlignmentsFromAlignmentIDList(
            self.alignmentIDList)
        
        method = self.db_main.getAlignmentDepthIntervalMethod(
            short_name=self.methodShortName,
            description=None, ref_ind_seq_id=None,
            individualAlignmentLs=alignmentList,
            parent_db_entry=None, parent_id=None, \
            no_of_alignments=None, no_of_intervals=None, \
            sum_median_depth=None, sum_mean_depth=None,\
            data_dir=self.data_dir)
        self.checkIfAlignmentListMatchMethodDBEntry(
            individualAlignmentLs=alignmentList,
        ...
        db_entry = self.db_main.getAlignmentDepthIntervalFile(
            alignment_depth_interval_method=method,
            alignment_depth_interval_method_id=None,
            path=None, file_size=None, \
            chromosome=self.chromosome,
            chromosome_size=inputFileData.chromosome_size,
            no_of_chromosomes=1,
            no_of_intervals=inputFileData.no_of_intervals,
            format=self.format,\
        ...
        if db_entry.id and db_entry.path:
            isPathInDB = self.db_main.isPathInDBAffiliatedStorage(
                relativePath=db_entry.path, data_dir=self.data_dir)
            if isPathInDB==-1:
                logging.error(f"updating AlignmentDepthIntervalFile.path with "
                    f"the new path, {db_entry.path}.")
                self.cleanUpAndExitOnFailure(exitCode=isPathInDB)
            elif isPathInDB==1:	#successful exit, entry already in db
                logging.warn("File %s is already in db."%(db_entry.path))
                session.rollback()
        ...
        relativePath = db_entry.constructRelativePath(

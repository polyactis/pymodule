...
Examples:
    ...
    %s --drivername mysql -u yh --hostname papaya --dbname genome --schema ""
        --commit  -b --report -g at mitochondrion1.genomic.fna.gz
    ...
    %s -u yh --schema genome --dbname vervetdb --drivername postgresql
        ...
        -i raw_sequence/wustl_gxfer1_78364318756303_ref6.0.3/supercontigs.fasta.gz
        --db_passwd secret --version 2
    #2013.04.12
    %s -u yh --schema genome --dbname vervetdb --drivername postgresql
        --commit -y4 --sequence_type_id 1 -z uclaOffice --tax_id 60711
        -x 10000
        -i raw_sequence/Chlorocebus_sabaeus_1.0/Chlorocebus_sabaeus_1.0.fasta
        --version 1
...
class chromosome_fasta2db(AbstractDBJob):
    ...
    option_default_dict.update({
        ('organism', 0, ): [None, 'g', 1, 
            '2-letter abbreviation for organism. Optional. '
            'If specified, only sequence from this organism would be extracted.'],\
        ('sequence_type_id', 0, int):[9, '', 1, 
            'column SequenceType.id in database GenomeDB'],\
        ('sequence_type_name', 0, ):[None, 's', 1, 'column SequenceType.short_name'],\
        ('tax_id', 0, int):[60711, '', 1, 
            'taxonomy ID, if not given, query argument organism against tax db'],\
        ...
        ('maxNoOfFastaRecords', 1, int):[500, 'x', 1, 
            'maximum number of fasta records to be inserted (in the input file order)'],\
        })
    ...
    def saveRawSequence(self, session, seq_to_db, passingdata, aa_attr_instance):
        ...
        raw_sequence = RawSequence(start=passingdata.current_start, 
            stop=passingdata.current_stop, sequence=seq_to_db)
...
>gi|285002488|gb|AC239185.3| Chlorocebus aethiops BAC clone CH252-404N12 from chromosome unknown, complete sequence
    ...
    def run(self):
        ...
        logging.info("\tTotally, %d files to be processed.\n"%(len(self.inputFnameLs)))
        ...
        for filename in self.inputFnameLs:
            ...
            self.parse_chromosome_fasta_file(db=self.db, filename=filename, 
                tax_id=self.tax_id, version=self.version, chunk_size=10000, \

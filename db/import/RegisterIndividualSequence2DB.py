...
Examples:
	...
	option_default_dict.update({
		...
		("sequence_type_name", 0, ): ["", '', 1, 
			'PairedEnd or SingleRead or others in table sequence_type.'],\
		("original_sequence_filepath", 0, ): ["", '', 1, 
			'path to the original sequence file'],\
		('original_sequence_mate_id', 0, int):[None, '', 1, 
			'mate_id of original_sequence, only valid if original_sequence contains only one end'],\
		('original_sequence_md5sum', 0, ): ["", '', 1, 
			'The md5sum for the original_sequence_filepath, not the input file '],\
		('original_sequence_library', 0,): [None, '', 1, 
			'library name for the original_sequence', ],\
		('copy_original_file', 0, int):[0, '', 1, 
			'toggle to copy original_sequence_filepath into db-affliated storage'],\

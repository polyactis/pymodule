...
class FindTablesWithLogColumns(object):
    ...
    option_default_dict = {
        ...
        ('hostname', 1, ): ['localhost', 'z', 1, 
            'hostname of the db server', ],\
        ('port', 0, int):[5432, '', 1, 
            'the database server port number on the host. '
            'Default mysql is 3306.'],\
        ('dbname', 1, ): ['graphdb', 'd', 1,
            'stock_250k database name', ],\
        ('schema', 0, ): [None, 'k', 1, 
    ...
    def run(self):
        ...
        for row in rows:
            ...
            try:
                ...
                #this table doesn't have these columns.
                self.establishConnection()
                # connection/transaction is gone due to exception 
                if self.debug:

...
create or replace view view_individual as select i.id, i.code, i.name, i.tax_id, i.sex, i.age, u.realname as collector, i.collection_date, i.date_created, i.date_updated, s.latitude, 
...
create or replace view view_alignment as select i.id as individual_id, i.code, i.study_id, i.tax_id, i.sex, i.age, 
    ...
    ia.path, ia.file_size, ia.total_no_of_reads, ia.parent_individual_alignment_id, ia.mask_genotype_method_id, ia.local_realigned,
    ia.reduce_reads,
...
select a1.study_id, a1.alignment_id AS tumor_alignment_id, a1.tissue_id AS tumor_tissue_id, 
  ...
  from (select * from view_alignment where tissue_id<10) as a1 full outer join (select * from view_alignment where tissue_id>=10) as a2 
...
create or replace view view_individual_sequence as select ntt.*, t2.batch_id_list, t2.coverage_list
    ...
    isq.filtered, isq.quality_score_format, isq.tissue_id,
...
select a1.study_id, a1.individual_sequence_id AS tumor_isq_id, a1.tissue_id AS tumor_tissue_id, a1.tissue AS tumor_tissue, 
  a1.PATH AS tumor_file_path, a1.coverage AS tumor_coverage,
  a2.individual_sequence_id AS normal_isq_id, a2.tissue_id, a2.tissue, a2.PATH, a2.coverage
  from view_individual_sequence a1, view_individual_sequence a2 
  where a1.individual_id=a2.individual_id AND a1.sequence_type_id=5 AND a2.sequence_type_id=5 and a1.tissue_id <10 and a2.tissue_id>=10 
...
create or replace view view_sequence_before_after_filter as select t2.*, visq.batch_id_list, visq.coverage_list from (select i.code, t1.*

...
class PlotVCFtoolsStat(PlotTrioInconsistencyOverFrequency, AbstractMatrixFileWalker):
    ...
    option_default_dict.update({
        ...
        ('tax_id', 0, int): [60711, '', 1, 
            'taxonomy ID of the organism from which to retrieve the chromosome info', ],\
        ('sequence_type_id', 0, int): [1, '', 1, 
            'sequence_type_id (annot_assembly) of the chromosomes, '
            'to retrieve the chromosome info. 1: assembledChromosome, 9: Scaffolds', ],\
        ('chrOrder', 0, int): [1, '', 1,
            'how to order chromosomes. 1: column genome_order in db; 2: by chromosome size, descending', ],\
        })
    option_for_DB_dict = {
    ...
    def run(self):
        ...
        chr2size = db_genome.getTopNumberOfChomosomes(contigMaxRankBySize=80000,
            contigMinRankBySize=1, tax_id=self.tax_id, \
            sequence_type_id=self.sequence_type_id)
        ...
        for inputFname in self.inputFnameLs:
            ...
            self.vcftoolsOutputStatFileWalker(inputFname, processFunc=None,
                chrColumnHeader=self.chrColumnHeader,\
                minChrLength=self.minChrLength,
                chrLengthColumnHeader=self.chrLengthColumnHeader,
                xColumnHeader=self.xColumnHeader,
                valueForNonPositiveYValue=self.valueForNonPositiveYValue)

...
Examples:
    ...
    %s  -I ~/NetworkData/vervet/db/genotype_file/method_42/
        ...
        -u yh -z localhost
        -o dags/FindNewRefCoordinates_Method42_vs_3231_maxContigID2.xml -x 2
        #-U 0 -Z 3000
    ...
    %s -I ~/NetworkData/vervet/db/genotype_file/method_42/ 
        ...
        -j hcondor -l hcondor -D ~/NetworkData/vervet/db/ -t ~/NetworkData/vervet/db/
        -u yh -z localhost
        -o dags/FindNewRefCoordinates_Method42_vs_3231_BWA.xml
        --alignmentMethodType 2
        --intervalSize 40000
        #--flankingLength 50
    ...
    #. mapEachVCF:
      ...
      #. mapEachInterval
         #. extract flanking sequences from the input VCF ():
             ExtractFlankingSequenceForVCFLoci.py -i vcf -a reference fasta
...
class FindNewRefCoordinatesGivenVCFFolderWorkflow(ParentClass, BlastWorkflow, \
    ...
    def preReduce(self, outputDirPrefix="", passingData=None, transferOutput=True, **keywords):
        ...
        if self.alignmentMethodType==1:	#blast
            ...
            registerReferenceData = self.registerBWAIndexFile(
            ...
            if registerReferenceData.needBWARefIndexJob:	#2013.3.20
                ...
                refIndexJob = self.addBWAReferenceIndexJob(
                    ...
                    refSequenceBaseCount=3000000000, bwa=self.bwa,\
                    transferOutput=True)
                #add the index files
                passingData.newRefFastaFileList = \
                    [passingData.newRefFastaFileList[0]] + refIndexJob.outputList
        ...
        self.mergeLocusLiftOverProbabilityJob = self.addStatMergeJob(
        ...
        sortProbabilityFile = File(os.path.join(self.reduceOutputDirJob.output,
            'locusLiftOverProbability.sorted.tsv.gz'))
        sortProbabilityJob = self.addSortJob(
            ...
            sshDBTunnel=None, job_max_memory=4000, walltime=120)
        ...
        #   needs to be changed to match the old, not new ref.
        ...
        self.addDrawHistogramJob(executable=self.DrawHistogram,
            inputFileList=[self.mergeLocusLiftOverProbabilityJob.output], \
            outputFile=outputFile, \
            whichColumn=None, whichColumnHeader="mapPvalue",
            whichColumnPlotLabel="minusLogLiftOverPvalue", \
        ...
        outputFile = File(os.path.join(self.reduceStatDirJob.output,
            'noOfLociChange.s5.AfterRemoveRedundancyMerge.stat.tsv'))
        self.noOfLociAfterRemoveRedundancyMergeJob = self.addStatMergeJob(
            statMergeProgram=self.ReduceMatrixByChosenColumn, \
        ...
        #divide the number to get reduction fraction
        outputFile = File(os.path.join(self.reduceStatDirJob.folder, 
            'reduction.s5.ByRemoveRedundancyPerContigMerge.stat.tsv'))
        ...
        outputFile = File(os.path.join(self.plotDirJob.folder, \
            'noOfSwitchPoints_vs_chromosomeSize.png'))
        #no spaces or parenthesis or any other shell-vulnerable letters in
        #  the x or y axis labels (whichColumnPlotLabel, xColumnPlotLabel)
        self.addAbstractPlotJob(executable=self.AbstractPlot, \
            inputFileList=[concatenateSwitchPointBySomethingJob.output], \
            outputFile=outputFile, \
            whichColumn=None, whichColumnHeader="noOfSwitchPoints",
            whichColumnPlotLabel="noOfSwitchPoints", \
        ...
        outputFile = File(os.path.join(self.plotDirJob.folder, \
            'fractionOfLociCovered_vs_maxSwitchFrequency.png'))
        #no spaces or parenthesis or any other shell-vulnerable letters in
        #  the x or y axis labels (whichColumnPlotLabel, xColumnPlotLabel)
        self.addAbstractPlotJob(executable=self.AbstractPlot, \
            inputFileList=[calculateLociGenomeLeftAtThresholdJob.output], \
    ...
    def addFindNewRefCoordinateJob(self, executable=None, inputFile=None, \
        ...
        findNewRefCoordinateJob = self.addAbstractMapperLikeJob(\
    ...
    def mapEachInterval(self,
        VCFJobData=None, intervalData=None, passingData=None,
        ...
        2012.10.3
            ...
            #. make a new VCF file based on the input split VCF file
                (replace contig ID , position with the new one's, remove the 
                    header part regarding chromosomes or replace it)
        ...
        outputFnamePrefix = os.path.join(mapDirJob.output,
            '%s_flankSequence'%(intervalFileBasenamePrefix))
        outputFile = File('%s.fasta'%(outputFnamePrefix))
        extraArgumentList = ['--flankingLength %s'%(self.flankingLength),
            '--refFastaFname', passingData.oldRefFastaFile,\
            "--outputFormatType %s"%(self.alignmentMethodType)]
        # alignmentMethodType 1 => fasta
        # alignmentMethodType 2 => fastq
        extractFlankSeqJob = self.addAbstractMapperLikeJob(
            executable=self.ExtractFlankingSequenceForVCFLoci, \
            inputF=VCFFile, outputF=outputFile, \
            parentJobLs=[mapDirJob]+VCFJobData.jobLs,
            transferOutput=transferOutput, job_max_memory=2000,\
            extraArguments=None, extraArgumentList=extraArgumentList,
            extraDependentInputLs=[passingData.oldRefFastaFile])
            ...
            fileObjectLs = [PassingData(fastqF=extractFlankSeqJob.output, \
                db_entry=PassingData(quality_score_format='Standard', \
                    individual_sequence=PassingData(sequencer=PassingData(short_name='GA'), \
                        sequence_type=PassingData(short_name='SR', 
                            read_length_mean=2*self.flankingLength, paired_end=0)),\
                        read_count=splitVCFJob.noOfSitesPerUnit))]
            bwaAdditionalAlignments = " -O 1000 -E 100"
            	#set Gap open and extension penalty really high to avoid gaps
            if self.alignmentMethodType==2:
                #2012.10.10 fake one
                alignment_method = PassingData(short_name='bwamem', command='mem')
                	#2013.05.21 use mem instead of aln
                bwaAdditionalAlignments += ' -a'
                #2013.11.29 Output all found alignments for single-end or unpaired paired-end reads.
        ...
        self.addInputToStatMergeJob(statMergeJob=self.switchPointStatMergeJob,
        ...
        outputFile = File(os.path.join(self.liftOverMapDirJob.output, 
            '%s.s1.extraInfo.vcf'%(intervalFileBasenamePrefix)))
        addInfoDescJob = self.addGenericJob(
            executable=self.AddMissingInfoDescriptionToVCFHeader, \
        ...
        unsortedVCFFile = File(os.path.join(self.liftOverMapDirJob.output, \
            '%s.s2.LiftOverUnsorted.vcf'%(intervalFileBasenamePrefix)))
        liftoverVariantsJob = self.addGenericJob(
            executable=self.LiftOverVCFBasedOnCoordinateMap, \
        ...
        unsortedVCFFile = File(os.path.join(self.liftOverMapDirJob.output,
             '%s.unsorted.vcf'%(intervalFileBasenamePrefix)))
        liftoverVariantsJob = self.addGATKJob(executable=self.LiftoverVariants,
            GATKAnalysisType="LiftoverVariants",\
            ...
            frontArgumentList=None, extraArguments=None, \
            extraArgumentList=["-chain", chainFile, "-dict",
            self.newRegisterReferenceData.refPicardFastaDictF, "-recordOriginalLocation"], \
            extraOutputLs=None, \
            extraDependentInputLs=None, \
            parentJobLs=[findNewRefCoordinateJob, self.liftOverMapDirJob, addInfoDescJob], \
        ...
        #  -R $newRef.fasta -V:variant $sorted_vcf -o $out";
        #
        filteredLiftOverVCFFile = File(os.path.join(self.liftOverMapDirJob.output, \
            '%s.s4.filteredLiftOver.vcf'%(intervalFileBasenamePrefix)))
        filterLiftoverVariantsJob = self.addGATKJob(
            executable=self.FilterLiftedVariants, \
        ...
        removeLocusFromVCFWithLowLiftOverMapPvalueJob = self.addGenericJob(
            ...
            outputFile=outputFile, outputArgumentOption="-o", \
            parentJobLs=[self.liftOverMapDirJob, clearVCFBasedOnSwitchDensityJob,
                computeLiftOverLocusProbJob], \
            extraDependentInputLs=None, extraOutputLs=None, \
            frontArgumentList=None, extraArguments=None, \
    ...
    def reduceEachVCF(self, chromosome=None, passingData=None, mapEachIntervalDataLs=None,\
        ...
        concatenateVCFJobData = self.concatenateIntervalsIntoOneVCFSubWorkflow(
            refFastaFList=self.newRegisterReferenceData.refFastaFList, \
            fileBasenamePrefix='chr.%s.%s'%(chromosome, passingData.fileBasenamePrefix),
    ...
    def reduce(self, passingData=None, reduceEachChromosomeDataLs=None, transferOutput=True, **keywords):
        ...
        returnData.jobDataLs.append(
            PassingData(jobLs=[sortSNPID2NewCoordinatesJob],
                file=sortSNPID2NewCoordinatesJob.output, \
        ...
        concatenateVCFJobData = self.concatenateIntervalsIntoOneVCFSubWorkflow(
            ...
            transferOutput=False, job_max_memory=job_max_memory,
            walltime=walltime, needBGzipAndTabixJob=False)
        ...
        for seqTitle in newRefFastaFile.seqTitleList:
            ...
            selectOneChromosomeVCFJob = self.addSelectVariantsJob(
                ...
                parentJobLs=concatenateVCFJobData.jobLs + [self.liftOverReduceDirJob],
                extraDependentInputLs=concatenateVCFJobData.fileLs, \
                transferOutput=False, \
                extraArguments=None, extraArgumentList=None, 
                job_max_memory=job_max_memory, walltime=walltime)
            ...
            #record how many sites are lost
            outputF = File(os.path.join(self.statDirJob.output, \
                '%s.noOfLociAfterRemoveRedundancy.tsv'%(seqTitle)))
            self.addVCFBeforeAfterFilterStatJob(chromosome=seqTitle, outputF=outputF, \
                vcf1=selectOneChromosomeVCFJob.output,
                currentVCFJob=removeRedundantLociFromVCFJob, \
                statMergeJobLs=[self.noOfLociAfterRemoveRedundancyMergeJob,\
                    self.noOfLociPerContigAfterRemoveRedundancyMergeJob],
                parentJobLs=[selectOneChromosomeVCFJob, 
                    removeRedundantLociFromVCFJob, self.statDirJob])
            ...
            vcfSorterJob = self.addPipeCommandOutput2FileJob(
                ...
                extraDependentInputLs=[self.newRegisterReferenceData.refPicardFastaDictF,
                    removeRedundantLociFromVCFJob.output], \
                extraOutputLs=None, transferOutput=False, \
                extraArguments=None,
                extraArgumentList=[self.newRegisterReferenceData.refPicardFastaDictF,
                    removeRedundantLociFromVCFJob.output], \

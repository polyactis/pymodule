...
class SplitPedigreeVCFIntoBeagleTriosDuosFiles(AbstractVCFMapper):
    ...
    gatkPrintBeagleFname is the output of this command:
        ...
        counter = 0
        real_counter = 0
        plinkPedigreeFile = PlinkPedigreeFile(path=plinkPedigreeFname,
            dummyIndividualNamePrefix=dummyIndividualNamePrefix)
        ...
        for node in pedigreeGraph:
            if node in sampleID2ColIndexList:
                ...
                memberList = []
                familySize = None
                if no_of_incoming_edges==0 and no_of_outgoing_edges==0:
                    #check the outgoing as well because founders have no incoming edges.
                    familySize = 1
    ...
    def checkConcordanceBetweenBeagleAndVCFCall(self, vcfGenotypeCall=None,
        diploidCallFromBeagle=None):
        """
        2013.05.06
    ...
    def run(self):
        ...
        if self.debug:
            import pdb
            pdb.set_trace()
        pedigreeFamilyData = self.constructPedigreeFamilyData(
            plinkPedigreeFname=self.plinkPedigreeFname,
            gatkPrintBeagleFname=self.gatkPrintBeagleFname)
        beagleFileData = self.openWriteBeagleFiles(
            pedigreeFamilyData=pedigreeFamilyData,
            outputFnamePrefix=self.outputFnamePrefix)
        
        self.splitVCFIntoBeagleInputs(inputFname=self.inputFname,
            beagleLikelihoodFile=pedigreeFamilyData.beagleLikelihoodFile, \
            familySize2BeagleFileHandler=beagleFileData.familySize2BeagleFileHandler, \
            pedigreeFamilyData=pedigreeFamilyData,
            minProbForValidCall=self.minProbForValidCall,\

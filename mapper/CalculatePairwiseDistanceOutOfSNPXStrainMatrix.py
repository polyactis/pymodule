...
Examples:
    #
    %s -i data/1MbBAC_as_ref/454_illu_6_sub_vs_1MbBAC.GATK.call
    -o /tmp/454_illu_6_sub_vs_1MbBAC.GATK.matrix -m 0.2 -n 0.4 -c1
...
# #to avoid popup and collapse in X11-disabled environment
...
class CalculatePairwiseDistanceOutOfSNPXStrainMatrix(object):
    ...
    option_default_dict = {
        ...
        ('min_MAF', 1, float): [0.0, 'n', 1, 
            'minimum MAF for SNP filter', ],
        ('max_NA_rate', 1, float): [0.4, 'm', 1, 
            'maximum NA rate for SNP filter', ],
        ('convertHetero2NA', 1, int):[0, 'c', 1, 
            'toggle convertHetero2NA mode'],
        ('hetHalfMatchDistance', 1, float): [0.5, 'H', 1, 
            'distance between two half-matched genotypes. AG vs A or AG vs AC'],
    ...
    @classmethod
    def calculatePairwiseDistanceOutOfSNPXStrainMatrix(cls, inputFname,
        outputFname, snpFnameToDoFiltering=None,
        ...
        if snpFnameToDoFiltering:
            oldSNPData = SNPData(input_fname=snpFnameToDoFiltering,
                turn_into_array=1, ignore_2nd_column=1,
                input_alphabet=1, turn_into_integer=1)
            snpData = transposeSNPData(oldSNPData)
            if convertHetero2NA:
                snpData = SNPData.convertHetero2NA(snpData)
            
            snpData = snpData.removeColsByNARate(snpData,
            ...
            oldSNPData = SNPData(input_fname=inputFname,
                turn_into_array=1,
                ignore_2nd_column=1,
                input_alphabet=1, turn_into_integer=1)
            snpData = transposeSNPData(oldSNPData)
        ...
        row_id2pairwise_dist_ls = snpData.calRowPairwiseDist(
            assumeBiAllelic=True, outputFname=outputFname,
            hetHalfMatchDistance=hetHalfMatchDistance)
        cls.outputRow_id2pairwise_dist_lsInMatrix(
            row_id2pairwise_dist_ls, outputFname)
    
    """
        #2011-4-7
        inputFname = os.path.expanduser('~/script/vervet/data/topConservedHG19_random_100loci_vervetSNP.tsv')
        
        inputFname = os.path.expanduser('hoffman2_home/script/vervet/data/1MbBAC_as_ref/454_illu_6_sub_vs_1MbBAC_minMAC3_maxMAC7_maxNoOfReadsForGenotypingError1.maxCoverage20.maxMajorAC10.maxNoOfReadsForAllSamples1000.snps')
        snpFnameToDoFiltering = os.path.expanduser(
            'hoffman2_home/script/vervet/data/1MbBAC_as_ref/454_illu_6_sub_vs_1MbBAC_minMAC3_maxMAC7_maxNoOfReadsForGenotypingError1.maxCoverage20.maxMajorAC10.maxNoOfReadsForAllSamples1000_no_carribean.snps')
        ...
        VariantDiscovery.calculatePairwiseDistanceOutOfSNPXStrainMatrix(inputFname,
            outputFname, snpFnameToDoFiltering=snpFnameToDoFiltering,
            convertHetero2NA=convertHetero2NA, min_MAF=min_MAF,
            max_NA_rate=max_NA_rate)
        sys.exit(0)
        
        #2011-7-8
        inputPrefix = os.path.expanduser("454/vs_MinSize200Scaffolds_by_bwasw")
        ...
        VariantDiscovery.filterAlignmentByReferenceIDs(inputFname,
            outputFname, referenceIDSet=referenceIDSet,
            readGroup='454_vs_Contig0')
        sys.exit(0)
    """
    
    
    @classmethod
    def outputRow_id2pairwise_dist_lsInMatrix(cls,
        row_id2pairwise_dist_ls, outputFname):
        """
        2011-5-14
        ...
        for row_id in row_id_ls:
            ...
            for dist in pairwise_dist_ls:
                mismatch_rate, row_id2, \
                    no_of_mismatches, no_of_non_NA_pairs = dist[:4]
    ...
    def run(self,):
        if self.debug:
            # enter debug mode "~/.../variation/misc.py -b"
            import pdb
            pdb.set_trace()
            debug = True
        else:
        ...
        max_NA_rate = 0.4
        min_MAF = 0
        outputFname ='%s.pairwiseDist.convertHetero2NA%s.minMAF%s.maxNA%s.tsv'%(
            os.path.splitext(self.inputFname)[0],
            convertHetero2NA, min_MAF, max_NA_rate)
        """
        self.calculatePairwiseDistanceOutOfSNPXStrainMatrix(
            self.inputFname, self.outputFname,
            snpFnameToDoFiltering=snpFnameToDoFiltering,
            convertHetero2NA=self.convertHetero2NA,
            min_MAF=self.min_MAF, max_NA_rate=self.max_NA_rate,

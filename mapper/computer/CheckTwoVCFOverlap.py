...
class CheckTwoVCFOverlap(AbstractVCFMapper):
    ...
    option_default_dict.update({
        ('jnputFname', 1, ): ['', 'j', 1,
            '2nd VCF input file. either plain vcf or gzipped is ok. '
            'could be unsorted.', ],\
        ('overlappingSitesOutputFname', 0, ): ['', '', 1,
            'output filename that would contain locations of all overlap '
            'sites between two VCF. no output if not given.'],\
        ('perSampleConcordanceOutputFname', 0, ): ['', '', 1, 
            'output filename for the per-sample matching result.'

...
class SampleRows(AbstractMatrixFileWalker):
    ...
    option_default_dict.update({
        ('sampleSize', 1, int): [None, '', 1, 
            'number of samples (rows) to be sampled from input,'
            'uniformly if plinkIBDCheckOutputFname is not given.'],\
        ('plinkIBDCheckOutputFname', 0, ): [None, '', 1, 
            'file that contains IBD check result, PI_HAT=relatedness. '
        ...
        ('maxIBDSharing', 1, float): [0.1, '', 1, 
            'This argument caps the maximum IBD sharing among any pair within the sampled.'],\
        })
    ...
    def setup(self, **keywords):
        ...
        if self.plinkIBDCheckOutputFname:
            ibdData = SNP.readAdjacencyListDataIntoMatrix(
                inputFname=self.plinkIBDCheckOutputFname,
            ...
            if self.ibdData:
                ...
                if col_name2index:
                    ...
                    while len(sampledRowIndexSet)<self.sampleSize:
                        ...
                        if randomIndex in sampledRowIndexSet:
                        ...
                        if whichColumn is not None:
                            ...
                            for sampledRowIndex in sampledRowIndexSet:
                                #not too close to anyone previously sampled

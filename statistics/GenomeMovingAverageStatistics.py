#!/usr/bin/env python3
"""
2013.07.31 a program that calculates moving averages in windows (overlap or not) 
    along the genome.

Examples:
    %s  -i  mendelRuntype3_s2Reduce/merged_lmendel_chr_pos.tsv 
    -o mendelRuntype3_s2Reduce/medianNoOfMendelErrors_along_chromosome.tsv
    --genome_drivername=postgresql --genome_hostname=localhost 
    --genome_dbname=vervetdb --genome_schema=genome
    --genome_db_user=yh --genome_db_passwd ...
    --chromosomeHeader Chromosome --tax_id 60711 --sequence_type_id 1 --chrOrder 1
    --samplingRate 1 --positionHeader Start --whichColumnHeader N 
    --valueForNonPositiveYValue -1 --inputFileFormat 1
...
class GenomeMovingAverageStatistics(ParentClass):
    ...
    option_default_dict.update({
        ...
        ('outputAverageColumnHeader', 0, ): ['score', '', 1, \
            'header for the output column that contains the averaged value'],\
        
        })
    def __init__(self, inputFnameLs=None, **keywords):
        """
        """
        #self.connectDB() called within ParentClass.__init__()
        ...
        meanPerBaseFunction = lambda ls: sum(ls)/float(self.windowSize+self.windowOverlapSize)
        ...
        for chromosome, chromosomeSize in self.oneGenomeData.chr_id2size.items():
            ...
            for i in range(no_of_intervals):
                ...
                segmentKey = CNVSegmentBinarySearchTreeKey(chromosome=chromosome, \
                            span_ls=[startPos, stopPos],\
                            min_reciprocal_overlap=1.0)
                
                interval = "%s:%s-%s"%(chromosome, originalStartPos, originalStopPos)
                intervalFileBasenameSignature = '%s_%s_%s'%(chromosome, \
                                            originalStartPos, originalStopPos)
                ...
                intervalData = PassingData(overlapInterval=overlapInterval, \
                    overlapIntervalFileBasenameSignature=overlapIntervalFileBasenameSignature,\
                    interval=interval, \
        ...
        for node in self.segmentKey2dataLsRBDict:
            for oneData in node.value:
                if len(oneData.dataLs)>0:
                    ...
                    data_row=  [node.key.chromosome, node.key.start, \
                                node.key.stop, len(oneData.dataLs), reduce_value]

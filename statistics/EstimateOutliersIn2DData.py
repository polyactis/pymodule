...
class EstimateOutliersIn2DData(AbstractPlot):
    ...
    def processHeader(self, header=None, pdata=None):
        ...
        if self.invariantPData.writer and not self.invariantPData.headerOutputted:
            newHeader = ["outputID", 'noOfOutliers', 'noOfNonMissing', \
                        'outlierFraction', 'chiSqStat', 'chiSqMinusLogPvalue',\
    ...
    def processRow(self, row=None, pdata=None):
        ...
        if col_name2index:
            ...
            if yValue!=self.missingDataNotation and xValue!=self.missingDataNotation:
                xValue = self.processValue(value=xValue, takeLogarithm=self.logX, \
                    positiveLog=self.positiveLog, \
                    valueForNonPositiveValue=self.valueForNonPositiveYValue)
                yValue = self.processValue(yValue, takeLogarithm=self.logY, \
                            positiveLog=self.positiveLog, \
                            valueForNonPositiveValue=self.valueForNonPositiveYValue)
    ...
    def reduce(self, **keywords):
        ...
        """
        meanStdData = statistics.estimateMeanStdFromData(dataVector=self.dataLs, \
                           excludeTopFraction=0.2)
        chiSqData = statistics.calculateChiSqStatOfDeltaVector(dataVector=self.dataLs, \
        ...
        dataRow = [thisOutputID, self.noOfOutliers, self.noOfNonMissing, \
                outlierFraction, chiSqData.chiSqStat,\
